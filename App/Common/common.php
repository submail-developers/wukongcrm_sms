...
function sendSMS($telphone, $message, $sign_name="sign_name",$sendtime=''){
    ...
    $data['appid'] = $sms['uid']; //短信平台帐号
    $data['signature'] = $sms['passwd']; //短信平台密码
    $data['to'] =   $telphone;
    $data['content'] = '【'.$sms[$sign_name].'】'.$message;
...
function sendtestSMS($uid, $uname, $telphone,$sign_name="sign_name"){
    ...
    $data['signature'] = $sms['passwd']; //短信平台密码
    $data['to'] =   $telphone;
    $data['content'] = '【'.$sms[$sign_name].'】'.'欢迎体验赛邮云短信！';
...
function sendGroupSMS($telphone, $message, $sign_name="sign_name",$sendtime=array()){
    $flag = 0;
    $sms = F('sms');
    //要post的数据
    $smsapi = "https://api.mysubmail.com/message/multisend/";
    $data['appid'] = $sms['uid']; //短信平台帐号
    $data['signature'] = $sms['passwd']; //短信平台密码
    $multi = explode(',',$telphone);
    foreach ($multi as $k=>$v){
        $data['multi'][$k]['to']    =   $v;
        if($sendtime){
            foreach ($sendtime as $kk=>$vv){
                if($kk==$k){
                    $data['multi'][$k]['vars']['name']=$vv;
                }
            }
        }
    }

    $data['multi']= json_encode($data['multi']);
    $data['content'] = '【'.$sms[$sign_name].'】'.$message;
    $query = http_build_query($data);
    $options['http'] = array(
        'timeout' => 60,
...
function getSmsNum(){
    ...
    $context = stream_context_create($options);
    $result = file_get_contents('https://api.mysubmail.com/balance/sms', false, $context);
    $output = trim($result, "\xEF\xBB\xBF");
    $res = json_decode($output, true);
    return $res['balance'];
}
...
function check_permission($module_id, $module, $permission_field='owner_role_id'){
    ...
    $permission_ids = getSubRoleId();
    if(in_array($owner_role_id, $permission_ids) || !$owner_role_id || in_array(intval(session('role_id')),$sharing_role_ids)){
        return true;
    }else{
        return false;
    }
